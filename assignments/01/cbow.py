...
def word2vec(dataset):
    ...
    with tf.name_scope('data'):
        ...
        around_words, center_words = iterator.get_next()
    ...
    with tf.name_scope('embed'):
        ...
        embed = tf.nn.embedding_lookup(embed_matrix, around_words, name='embedding')
...
def main():
    dataset = tf.data.Dataset.from_generator(gen,
                                             (tf.int32, tf.int32),
                                             (tf.TensorShape([BATCH_SIZE, 2 * SKIP_WINDOW]),
